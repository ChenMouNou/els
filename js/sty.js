		...
		k.cssHooks[u] = {
			...
			set: function(e, t, n) {
				...
				return a && o && (s -= Math.ceil(e["offset" + u[0].toUpperCase() + u.slice(1)] -
					...
					t)) && "px" !== (r[3] || "px") && (e.style[u] = t, t = k.css(e, u)), Ze(0,
					t, s)
			}
		}
						...
						return function() {
							o && (o = a = r.onload = r.onerror = r.onabort = r.ontimeout = r
								...
								"error" === e ? "number" != typeof r.status ? t(0,
									"error") : t(r.status, r.statusText) : t(Ut[r.status] ||
									r
...
function lunbo() {
	nav[pos].style.display = "none";
	pos++;
	if (pos == nav.length) pos = 0;
	nav[pos].style.display = "block";
	nav[pos].style.animation = "3s";
}
...
document.addEventListener('scroll', function() {
	var ul = document.querySelector('.bqbox')
...
var list = document.querySelector('.biaoq');
var text = document.querySelector('.serchbox');
...
move.onclick=function(){
		 window.open('move.html',"new","width=640,height=800,top=0,left=100");
}
